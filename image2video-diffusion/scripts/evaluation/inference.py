...
def save_results_seperate(prompt, samples, filename, fakedir, fps=10, loop=False):
    ...
    for idx, video in enumerate(videos):
        ...
        for i in range(n):
            ...
            # Save directly to main output directory (no subfolders)
...
def run_inference(args, gpu_num, gpu_no):
    ...
    start = time.time()
    print(f"🚀 Starting generation: {len(prompt_list_rank)} video(s) with {n_frames} frames each")
    print(f"⏱️  Expected time: ~{args.ddim_steps * len(prompt_list_rank) * 0.5:.0f}s for 256 model, ~{args.ddim_steps * len(prompt_list_rank) * 3:.0f}s for 1024 model")
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            for nn, samples in enumerate(batch_samples):
                ...
                save_results_seperate(prompt, samples, filename, output_dir, fps=8, loop=args.loop)
